        ...
        <div class="mt-8 sm:mx-auto sm:w-full sm:max-w-md">
            <div class="bg-white py-8 px-4 shadow sm:rounded-lg sm:px-10">
                ...
                <a href="{{ route('google.login') }}" 
                    class="flex w-full items-center justify-center gap-3 rounded-md border border-gray-300 bg-white py-2.5 px-4 text-sm font-medium text-gray-500 shadow-sm hover:bg-gray-50 focus:outline-none focus:ring-2 focus:ring-neutral-500 focus:ring-offset-2">
                ...
                <button type="button" onclick="toggleLoginForm()" style="cursor: pointer;"
                    ...
                    <span id="toggleButtonText">Sign in with Email</span>
                    <svg id="toggleButtonIcon" class="ml-2 h-4 w-4 transition-transform duration-200" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor">
                        <path fill-rule="evenodd" d="M5.23 7.21a.75.75 0 011.06.02L10 11.168l3.71-3.938a.75.75 0 111.08 1.04l-4.25 4.5a.75.75 0 01-1.08 0l-4.25-4.5a.75.75 0 01.02-1.06z" clip-rule="evenodd" />
                    </svg>
                </button>

                <!-- Email Password Form -->
                <form id="emailLoginForm" class="hidden space-y-6 mt-6" action="{{ route('login') }}" method="POST">
                    @csrf
                    <div>
    ...
    <script>
        function toggleLoginForm() {
            ...
            if (form.classList.contains('hidden')) {
                ...
                text.textContent = 'Hide Email Form';
            } else {
                form.classList.add('hidden');
                icon.classList.remove('rotate-180');
                text.textContent = 'Sign in with Email';
            }
        }
    </script>
</body>
